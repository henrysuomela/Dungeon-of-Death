...
def fight_monster(monster_template):
    ...
    while monster['health'] > 0 and ps.player['health'] > 0:
        action = input("Press Enter to attack.\n")

        if action == "":
            player_damage = random.randint(0, ps.player['max_hit'])
            ...
            if monster['health'] > 0:
                ...
                if ps.player['armor_hp'] > 0:
                    if monster_damage >= ps.player['armor_hp']:
                        pure_damage = monster_damage - ps.player['armor_hp']
                        ps.player['armor_hp'] = 0
                        armor_destroyed_check = True
                        ps.player['health'] -= pure_damage
                    else:
                        ps.player['armor_hp'] -= monster_damage
                else:
                    ps.player['health'] -= monster_damage
                ...
                if ps.player['health'] > 0:
                    ...
                    if ps.player['armor_hp'] > 0:
                        print(f"\033[1;36m{ps.player['armor']}'s\033[0m remaining protection: \033[1;96m{ps.player['armor_hp']}\033[0m\n")
                    elif armor_destroyed_check:
                        print(f"Your \033[1;36m{ps.player['armor'].lower()}\033[0m loses its integrity!\n")
                    else:
                        print()
                else:
...
def compelling_choices():
    while True:
        ...
        if potion_choice == "y":
            health_before_heal = ps.player['health']
            if ps.player['health'] <= 75:
                ps.player['health'] += 25
            else:
                ps.player['health'] = 100
            heal_amount = ps.player['health'] - health_before_heal
...
def exit_encounter():
    ...
    print(
        ...
        f"\033[1;33m{ps.player['gold']} gold coin{'' if ps.player['gold'] == 1 else 's'}\033[0m in your pouch."
    )
