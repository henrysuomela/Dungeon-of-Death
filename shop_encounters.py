...
def tanky_shop():
    ...
    player_funds = sum(ps.player['inventory'].values())

    print(f"\nYou have \033[1;32m{player_funds} monster parts\033[0m in your bag.")
    print(f"Current armor: \033[1;36m{ps.player['armor']}\033[0m (remaining protection: \033[1;31m{ps.player['armor_hp']}\033[0m)\n")
    ...
    while True:
        ...
        if choice == "":
            ...
            while bill < item['cost']:
                monster_part = random.choice(list(ps.player['inventory'].keys()))
                if ps.player['inventory'][monster_part] > 1:
                    ps.player['inventory'][monster_part] -=1
                else:
                    del ps.player['inventory'][monster_part]
                bill += 1

            print(f"\033[1;33mThe merchant picks the {item['cost']} monster parts that call to him from your bag.\033[0m\n")

            ps.player['armor_hp'] = item['hp']
            if ps.player['armor'] == item['name']:
                print(f"You replace your \033[1;36m{ps.player['armor']}\033[0m with a fresh one.")
            else:
                ps.player['armor'] = item['name']
...
def mythical_shop():
    ...
    print(f"\nYou have \033[1;33m{ps.player['gold']} gold coins.\033[0m")
    print(f"Current weapon: \033[1;36m{ps.player['weapon']}\033[0m (Max Hit: \033[1;31m{ps.player['max_hit']}\033[0m)\n")
    ...
    while True:
        ...
        if choice == "":
            ...
            print("Choose the item you want to buy or type nothing and press Enter to leave.\n")
        elif shop_items[choice]['name'] == ps.player['weapon']:
            print("You're already wielding that weapon, the vendor suggests you pick something else!\n")
        else:
            item = shop_items[choice]

            if ps.player['gold'] < item['cost']:
                print("You don't have enough gold to buy that item.\n")
                continue

            ps.player['gold'] -= item['cost']
            ps.player['weapon'] = item['name']
            ps.player['max_hit'] = item['max_hit']

            print(f"\nYou have purchased the \033[1;36m{item['name']}\033[0m!")
            print(f"Your new weapon increases your max hit to \033[1;31m{ps.player['max_hit']}\033[0m.\n")
            print(f"You have \033[1;33m{ps.player['gold']} gold coin{'' if ps.player['gold'] == 1 else 's'}\033[0m left.")
            break