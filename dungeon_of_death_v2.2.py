...
def show_map():
    n = 1
    for x_axis in dungeon:
        print(n, x_axis)
        n += 1
    y, x = player['position']
    print(f"Current room: {dungeon[y][x]} (row {y+1}, room {x+1})")
"""
...
def show_map():
    ...
    for i, row in enumerate(dungeon):
        ...
        for j, room in enumerate(row):
            if i == y and j == x:
                room_display = f'▶ {room:<15}'
            elif visited[i][j]:
                room_display = f'{room:<18}'
            else:
                room_display = f'{"???":<18}'
            row_display.append(room_display)
        print(f'{i+1:2} | ' + ' | '.join(row_display))

    print(f"\nCurrent room: {dungeon[y][x]} (row {y+1}, room {x+1})")
